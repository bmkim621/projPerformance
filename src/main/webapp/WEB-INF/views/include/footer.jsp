<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<div class="familySiteLinkWrap">
	<ul>
		<li>
			<a href="#">
				<img src="${pageContext.request.contextPath }/resources/upload/link1.png">
			</a>
		</li>
		<li>
			<a href="#">
				<img src="${pageContext.request.contextPath }/resources/upload/link2.png">
			</a>
		</li>
		<li>
			<a href="#">
				<img src="${pageContext.request.contextPath }/resources/upload/link3.png">
			</a>
		</li>
		<li>
			<a href="#">
				<img src="${pageContext.request.contextPath }/resources/upload/link4.png">
			</a>
		</li>
	</ul>  
</div>

<!-- footer -->
<footer>
	<div class="container">
		<div class="row text-center sns-menu">
			<ul class="list-inline"> 
				<li>
					<a href="#"><i class="fab fa-facebook fa-lg"></i></a> 
				</li>        
				<li>
					<a href="#"><i class="fab fa-instagram fa-lg"></i></a>
				</li>         
				<li>
					<a href="#"><i class="fas fa-play-circle fa-lg"></i></a> 
				</li>             
				<li>
					<a href="#"><i class="fas fa-video fa-lg"></i></a>
				</li>      
			</ul>
		</div>
	             
	<div class="row text-center footer-menu">   
		<ul class="menu list-inline">
			<li>
				<a href="#">개인정보처리방침</a>
			</li>                       
			<li>
				<a href="#">이용약관</a>
			</li>                   
			<li>
				<a href="#">행정서비스현장</a>
			</li>                  
			<li>
				<a href="#">오시는 길</a>
			</li>
		</ul>
	</div>	<!-- 푸터 메뉴 끝 text-center end -->
	</div> <!-- container end -->
</footer>

<div class="copyright">
	<div class="container copyright-menu">
		<div class="row text-center copyright-info">
			<p class="contentWrap">
				<span id="addressSpan">우)41585 대구광역시 북구 호암로 15</span>
				<span id="telSpan">T.053-666-6000</span>
				<span id="faxSpan">F.053-666-6019</span>
			</p>
			<p class="contentWrap"> 
				<span id="copyRightSpan">Copyright©2018 Daegu Opera House. All Rights reserved.</span>
			</p>
			<p class="contentWrap">
				<span id="textSapn">(재)대구오페라하우스는 공직자윤리법 제3조의2에 따른 공직유관단체로서 부정청탁 및 금품 등 수수의 금지에 관한 법률(청탁금지법)적용대상 기관임을 알립니다.</span>
			</p> 
		</div>
	</div>
</div>

</body>
</html>